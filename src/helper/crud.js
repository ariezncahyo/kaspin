...
export default class Crud {
  ...
  async getDataFacade(req, res, next) {
    try {
      ...
      if (!data) return response.notFound(`There's no data ${this.module}`, res, flag.request.empty_data)
  ...
  async getDataByIDFacade(req, res, next) {
    try {
      ...
      if (!data) return response.notFound(`There's no data ${this.module} for this id`, res, flag.request.empty_data)
